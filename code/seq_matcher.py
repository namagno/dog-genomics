...
def load_alignment_get_distance(file:str):
    ...
    ### Function for loading the given multiple sequence alignment data and outputting DNA distance in a matrix format
    ###
    ### Input:
    ...
    # Set distance calculator as a variable
...
def distance_matrix_to_df(dm):   
    ...
    # Replace all 0.0000 in the df with NaN to avoid identical matches with identical sequences
    ...
    # Create a new column that returns a dog ID of the closest distance for each row respectively
...
def prepare_seqs(msa):
    ...
    ### Function for preparing the indentified seqences of interest
    ###
    ### Input:
    ###        Our profile-multiple sequence alignment file (note: dog IDs of interest were pre-determined from our df matching) 
    ###
    ### Output:
    ###        Individual mystery and target sequence fasta files saved in output folder
      
    # Read the multiple sequence alignement file and store it in a variable
    ...
    # Because we cannot feed lists into a pairwise aligner, we write them to new fasta files in the output folder 
...
def pairwise_alignment():
    ...
    ###
    ### Input:
    ###        Our mystery sequence and its closest target match determined by the distance matrix df
    ###
    ### Output:
    ###        A pairwise sequence alignment, text file written into output folder to observe differences. Number of matched nucleotides in alignment are displayed
    ...
    # Read fasta sequences and store them as input variables
    input_1 = SeqIO.read(myst_seq, "fasta")
    input_2 = SeqIO.read(targ_seq, "fasta")
    
    # Test to ensure sequences are the same length for alignment, then print length
    ...
    # Initialise our aligner parameters for global alignment and scoring (used as a nucleotide match counter in this case)
...
def construct_phylo_tree(cal_dist, dm):  
    ...
    ### Function for constructing a phylogenetic tree for the multiple sequence alignment distance matrix
    ###
    ### Input:
    ...
    # Plot a neighbour-joining tree from constructor and distance matrix, constrained layout minimises plot overcrowding
    ...
    # Get current figure so it can save to a file
    fig1 = plt.gcf()
    ...
    # Save phylogeny tree plot as .png in the output folder (Stretch Goal 2)
    fig1.savefig(save_results_to + 'phylogenetic_tree.png', dpi=300, bbox_inches='tight')

    # Print a message to prompt checking the phylo tree image generated in output folder
...
def draw_ascii_tree(nj_tree):  
    ...
    ### Function for presenting a basic ascii representation of our tree on the terminal/console
    ###
    ### Input:
